...
export function FAQAccordion({ items }: { items: IFAQ[] }) {
  return (
    <Accordion type="single" collapsible defaultValue="item-0" className="space-y-4 mt-10">
      {items.map((item, index) => (
        <AccordionItem key={index} value={`item-${index}`} className="bg-zinc-900 rounded-lg border border-zinc-700">
